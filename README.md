...
- `models`: Contains all the code used to create both the models, in particular in the `realTime` subdirectory it is possible to find one of the actual models created and a validation pcap file to check for the results Note: if the training algorithm of the realTime model is executed, it will overwrite the optimally trained model provided.
